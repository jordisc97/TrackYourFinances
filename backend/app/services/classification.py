...
def _merchant_key(description: str, merchant: str) -> str:
    return (merchant or description or "").strip().lower()[:255]
...
def classify_uncategorized(db: Session, household_id: int, account_ids: list[int], use_llm: bool = True) -> int:
    ...
    if use_llm and llm_queue:
        updated += _classify_with_llm_batches(db, household_id, llm_queue, categories_by_name)
    updated += _assign_leftover_positives_as_transfer(db, household_id, account_ids)
    db.commit()
    return updated


def _classify_with_llm_batches(db: Session, household_id: int, txs: list[Transaction], categories_by_name: dict[str, Category]) -> int:
